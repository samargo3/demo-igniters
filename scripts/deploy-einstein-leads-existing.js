...
console.log('🚀 Starting Einstein-optimized lead import using existing process...\n');
...
// Split into batches of 200 (same as your existing process)
...
for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
  ...
  // Create Apex script for this batch with Einstein fields
  const apexScript = createEinsteinApexScript(batch, header, batchNumber);
...
function createEinsteinApexScript(batch, header, batchNumber) {
...
for(Integer i = 1; i < lines.size(); i++) {
    ...
    if(String.isNotBlank(line)) {
        ...
        if(fields.size() >= 20) {
            ...
            // Einstein-optimized custom fields
