...
console.log('🚀 Starting final import of 1000 leads...\n');
...
for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
  ...
  // Use Bulk API to upsert (without external ID)
  try {
    console.log(`📤 Importing Batch ${batchNumber} via Bulk API...`);
    const result = execSync(`sfdx force:data:bulk:upsert --sobject Lead --file ${batchCsvPath}`, { 
      stdio: 'pipe',
      encoding: 'utf8'
    });
