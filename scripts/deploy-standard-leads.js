...
console.log('🚀 Starting standard lead import with Einstein-optimized data...\n');
...
// Split into batches of 50 (same as your existing working process)
const BATCH_SIZE = 50;
const batches = [];
...
for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
  ...
  // Create Apex script for this batch (only standard fields)
  const apexScript = createStandardApexScript(batch, header, batchNumber);
...
console.log(`🎉 Standard lead import completed!`);
...
function createStandardApexScript(batch, header, batchNumber) {
  const csvData = header + '\n' + batch.join('\n');
  
  return `// Batch ${batchNumber} - Import ${batch.length} high-tech B2B leads
...
for(Integer i = 1; i < lines.size(); i++) {
    ...
    if(String.isNotBlank(line)) {
        ...
        if(fields.size() >= 15) {
            ...
            // Standard Lead fields (all available for Einstein Lead Scoring)
...
if(!leadsToInsert.isEmpty()) {
    try {
        ...
        System.debug('Successfully inserted ' + leadsToInsert.size() + ' high-tech B2B leads (Batch ${batchNumber})');
    } catch(Exception e) {
